...
controlPoints = flow["flow_model"]["controlPoints"]['controlPoint']

reservoirs = flow["flow_model"]["reservoirs"]['reservoir']
...
#create a function that reads the control point classifier and sorts
...
#this is where we will load in the streamflow data (input)
...
#            should I add a name attribute here?
        maxSpillway_flow = np.interp(poolElevation,name.Spillway['pool_elev_m'],name.Spillway['release_cap_cms'])
        #should I add a name attribute? i.e. name.maxSpillway_flow
        return (maxRO_flow,maxSpillway_flow)


#this is where the GetResOutflow function should be 


#add in condition about res type, the following function only applies if restype = storage
        

def AssignReservoirOutletFlows(name,outflow): #outflow is the output of the above fnt
    #flow file has a condition on reservoir not being null...dk if we need that here
    
    #reset gate values to 0.0
    powerFlow = 0.0
    RO_flow = 0.0
    spillwayFlow = 0.0
    ...
    if outflow < name.maxPowerFlow: #this value is stored
        ...
        if excessFlow <= name.maxRO_flow:
            ...
            if spillwayFlow < name.minSpillwayFlow: #where do I find this value?
                spillwayFlow = name.minSpillwayFlow
                RO_flow -= name.minSpillwayFlow - excessFlow
            if spillwayFlow > name.maxSpillwayflow:
                print('Flow: Maximum spillway volume exceed') #need to add a pct value here & the name and ID value
...
##why are max power flow & pool elevation inversely correlated?
    #min release val under composite file is also the max powerflow?
    
    #i need location of min spillway, max spillway, minRO, etc...also the gate values ??
