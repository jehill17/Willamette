...
MTDOUBLE Reach::m_mvREACH_H2O = 0;
...
bool ModelOutput::Init( MapLayer *pIDULayer )
   ...
   if ( m_queryStr.GetLength() > 0 )
      ...
      if ( gpModel->m_pMapExprEngine == NULL )
         ...
         gpModel->AddModelVar( "SWE Depth (mm)",      "hruVolSwe",     &HRU::m_mvDepthSWE_mm );   //
         gpModel->AddModelVar("Topsoil moisture (mm)", "hruVolTopSoilH2O", &HRU::m_mvTopSoilH2O_mm);
         gpModel->AddModelVar("Shallow Groundwater (mm)", "hruVolSGW", &HRU::m_mvShallowGW_mm);
         gpModel->AddModelVar("Deep Groundwater (mm)", "hruVolDGW", &HRU::m_mvDeepGW_mm);
         gpModel->AddModelVar("Total Precip (mm)", "hruPrecip", &HRU::m_mvCurrentPrecip);
         gpModel->AddModelVar("Rain Thrufall (mm)", "hruRainThrufall", &HRU::m_mvRainThrufall_mm);
         gpModel->AddModelVar("Rain Evap (mm)", "hruRainEvap", &HRU::m_mvRainEvap_mm);
         gpModel->AddModelVar("Snow Thrufall (mm)", "hruSnowThrufall", &HRU::m_mvSnowThrufall_mmSWE);
         gpModel->AddModelVar("Snow Evap (mm)", "hruSnowEvap", &HRU::m_mvSnowEvap_mmSWE);
         ...
         gpModel->AddModelVar("INSTRM_WRQ", "INSTRM_WRQ", &Reach::m_mvInstreamWaterRightUse);
         gpModel->AddModelVar("Q_DIV_WRQ", "Q_DIV_WRQ", &Reach::m_mvQ_DIV_WRQ);
         gpModel->AddModelVar("INSTRM_REQ", "INSTRM_REQ", &Reach::m_mvINSTRM_REQ);
         gpModel->AddModelVar("REACH_H2O", "REACH_H2O", &Reach::m_mvREACH_H2O);
         gpModel->AddModelVar("RESVR_H2O", "RESVR_H2O", &Reach::m_mvRESVR_H2O);
         gpModel->AddModelVar("HRU_TO_AQ (mm)", "HRU_TO_AQ", &HRU::m_mvHRU_TO_AQ_mm);

         gpModel->AddModelVar("Snowmelt (mm SWE)", "hruMelt", &HRU::m_mvMelt_mm); 
         gpModel->AddModelVar("Refreezing (mm)", "hruRefreezing", &HRU::m_mvRefreezing_mm);
         gpModel->AddModelVar("Infiltration (mm)", "hruInfiltration", &HRU::m_mvInfiltration_mm);
         gpModel->AddModelVar("Recharge to Irrigated Soil (mm)", "hruRechargeIrrigated", &HRU::m_mvRechargeToIrrigatedSoil_mm);
         gpModel->AddModelVar("Recharge to NonIrrigated Soil (mm)", "hruRechargeNonIrrigated", &HRU::m_mvRechargeToNonIrrigatedSoil_mm);
         gpModel->AddModelVar("Recharge to Plant Accessible Soil (mm)", "hruRechargeTopSoil", &HRU::m_mvRechargeTopSoil_mm);
         gpModel->AddModelVar("Recharge to Upper Groundwater (mm)", "hruRechargeUpperGW", &HRU::m_mvRechargeToUpperGW_mm);
         gpModel->AddModelVar("Q0 (mm)", "hruQ0", &HRU::m_mvQ0_mm);
         gpModel->AddModelVar("Q2 (mm)", "hruQ2", &HRU::m_mvQ2_mm);
         gpModel->AddModelVar("Percolation (mm)", "hruPercolation", &HRU::m_mvPercolation_mm);

         gpModel->AddModelVar("Recharge (mm)", "hruGWRecharge", &HRU::m_mvCurrentRecharge);
         gpModel->AddModelVar("GW Loss (mm)", "hruGWOut",      &HRU::m_mvCurrentGwFlowOut);
         ...
         gpModel->AddModelVar("Cumu. GW Out (mm)",     "hruCumGWOut",        &HRU::m_mvCumGwFlowOut);
         gpModel->AddModelVar("Cumu. GW Recharge (mm)", "hruCumGWRecharge",  &HRU::m_mvCumRecharge);

         gpModel->AddModelVar( "WC",                   "hruLayerWC",     &HRULayer::m_mvWC );
         gpModel->AddModelVar( "WC",                   "hruLayerWDepth", &HRULayer::m_mvWDepth);
         gpModel->AddModelVar( "Elws",                 "hruElws",        &HRU::m_mvElws);
...
int HRU::AddLayers( int soilLayerCount, int snowLayerCount, int vegLayerCount, float initWaterContent, float initTemperature, bool grid )
   ...
   while (i_soil < soilLayerCount)
      ...
      if (gpModel->m_initWaterContent.GetCount()==1)
         ...
         m_layerArray[layer]->m_volumeWater=atof(gpModel->m_initWaterContent[i_soil])*pHRULayer->m_depth*0.4f*m_area;
         }
      ...
      for ( int j=0; j < rows; j++ )
         ...
         if ( pConstraint == NULL && filename.CompareNoCase( "Missing" ) != 0  ) // not yet loaded?  and not 'Missing'?    Still not loaded?
            ...
            if ( lstrcmpi( control, "cp" ) == 0 )
               records = gpModel->LoadTable(gpModel->m_path+ m_dir+m_cpdir+filename, (DataObj**) &(pConstraint->m_pRCData), 0 );
            else
               records = gpModel->LoadTable( gpModel->m_path+m_dir+m_rpdir+filename, (DataObj**) &(pConstraint->m_pRCData), 0 ); 
...
FlowModel::FlowModel()
 ...
 , m_randNorm1(1,1,1)
   {
   gpModel = this;
   gpFlow->AddInputVar( "Climate Scenario", m_currentFlowScenarioIndex, "0=MIROC, 1=GFDL, 2=HadGEM, 3=Historic" );    
...
int FlowModel::SaveDetailedOutputGrid(CArray< FILE*, FILE* > &filePtrArray)
   {
   ASSERT(filePtrArray.GetSize() == 14);
   ...
   for (int i = 0; i < numCols;i++)
      ...
      for (int j = 0; j < numRows; j++)
         ...
         if (value == m_pGrid->GetNoDataValue())
            ...
            float l1 = 0.0f; float l2 = 0.0f; float l3 = 0.0f; float l4 = 0.0f; float l5 = 0.0f;

            HRULayer *pLayer = pHRU->GetLayer(0);
            float area = pHRU->m_area;
            float precip = pLayer->m_waterFluxArray[FL_TOP_SOURCE] / area*1000.0f;//precipitation (m3/d converted to mm/d )
            ...
            HRULayer *pLayer2 = pHRU->GetLayer(2);//Layer above Groundwater...specific to 5 Layer SRS model
            ...
            fwrite(&recharge, sizeof(float), 1, filePtrArray[7]);//mm/d where + means gw is recharging and - means gw is discharging.
...
int FlowModel::SaveState()
   ...
   FILE *fp;
   CString filename;
   filename.Format("%s%s", (LPCTSTR)tmpPath, (LPCTSTR)m_initConditionsFileName);
   ...
   if (PathManager::FindPath(m_initConditionsFileName, tmpPath) < 0) //the file cannot be found, so skip the read and set flag to write file at the end of the run
     ...
     /* fp=fopen(file, "rb");
      if (fp == NULL)
         {*/
         CString msg;
         msg.Format("Unable to open file %s.  Initial conditions won't be saved.  ", tmpPath);
         Report::LogMsg(msg, RT_WARNING);
         fclose(fp);
         return -1;
         }
...
bool FlowModel::Run( EnvContext *pEnvContext )
   ...
   if ( !m_useStationData )
      {
      LoadClimateData(pEnvContext, pEnvContext->weatherYear );
      }
   ...
   while ( ( m_currentTime + TIME_TOLERANCE ) < m_stopTime )
      ...
      msg.Format("%s %i, %i %f", ::GetMonthStr(m_month), m_day, this->GetCurrentYear(), m_currentTime);
      ::EnvSetLLMapText( msg );
...
bool FlowModel::StartYear( FlowContext *pFlowContext )
   ...
   /*
   // fix up irrigated/non-irrigated if needed
   int irrLayerIndex = -1;
   for ( int i=0; i < m_hruLayerNames.GetSize(); i++ )
      {
      if ( m_hruLayerNames[ i ].CompareNoCase( "IrrigatedSoil") == 0 )
         { irrLayerIndex = i; break; }
      }
   
   int unIrrLayerIndex = -1;
   for ( int i=0; i < m_hruLayerNames.GetSize(); i++ )
      {
      if ( m_hruLayerNames[ i ].CompareNoCase( "UnirrigatedSoil") == 0 )
         { unIrrLayerIndex = i; break; }
      }

   if ( pFlowContext->pEnvContext->yearOfRun > 0 && irrLayerIndex >= 0 && unIrrLayerIndex >= 0 )
      {
      ...
      for (int h = 0; h < hruCount; h++)
         ...
         for (int m = 0; m < pHRU->m_polyIndexArray.GetSize(); m++)
            {
            int idu = pHRU->m_polyIndexArray[m];
            
            float area = 0.0f;
            int irrigated = 0;
            m_pCatchmentLayer->GetData( idu, m_colIrrigation, irrigated);
         ...
         //     change      =    "new" ares          "old" area
         float irrAreaDelta = hruIrrigatedArea - pHRU->m_areaIrrigated;    //if this number is +, then we gained that much irrigated area

         if ( fabs( irrAreaDelta ) > 1 )     // was there a change in irrigation area? then redistribute water between the irrig and non-irrig pool
            {
            // three cases - 1) start with irrigation area, end with no irrigated area
            //               2) start with no irrigated area, end with irrigated area
            //               3) start with some, end with some.

            float volUnIrrigated = (float) pHRU->GetLayer(unIrrLayerIndex )->m_volumeWater;
            float volIrrigated = (float) pHRU->GetLayer(irrLayerIndex)->m_volumeWater;

            // case (1) move all water from irrigate area to non-irrigated area
            if ( pHRU->m_areaIrrigated > 0.1f && hruIrrigatedArea < 0.1f )
               {
               pHRU->GetLayer( irrLayerIndex )->m_volumeWater = 0;
               pHRU->GetLayer( unIrrLayerIndex )->m_volumeWater = volIrrigated;
               }

            // case (2) - move all water from non-irrigated to irrigated
            else if ( pHRU->m_areaIrrigated < 0.1f && hruIrrigatedArea  > 0.1f )
               {
               pHRU->GetLayer( unIrrLayerIndex )->m_volumeWater = 0;
               pHRU->GetLayer( irrLayerIndex )->m_volumeWater = volUnIrrigated;
               }

            // case (3) - redistribute based on change of area
            else
               {
               if ( irrAreaDelta > 0 )    // are we moving water from non-irrigated to irrigated?
                  {
                  // calculate volume lost in non-irrigated
                  float areaUnIrr = pHRU->m_area - pHRU->m_areaIrrigated;
                  float volTransferred = volUnIrrigated * ( irrAreaDelta / areaUnIrr );

                  pHRU->GetLayer( unIrrLayerIndex )->m_volumeWater -= volTransferred;
                  pHRU->GetLayer( irrLayerIndex   )->m_volumeWater += volTransferred;
                  }
               else    // moving water from irrigated to nonirrigated
                  {
                  // calculate volume lost in non-irrigated
                  float volTransferred = volIrrigated * ( -irrAreaDelta / pHRU->m_areaIrrigated );
            ...
            // end of three cases - fix up water contents as needed
            pHRU->m_areaIrrigated = hruIrrigatedArea;

            pHRU->GetLayer( unIrrLayerIndex )->m_wc = float(pHRU->GetLayer(unIrrLayerIndex)->m_volumeWater / (pHRU->m_area - pHRU->m_areaIrrigated)) * MM_PER_M;    // wc = mm
            pHRU->GetLayer( irrLayerIndex   )->m_wc = float(pHRU->GetLayer(irrLayerIndex  )->m_volumeWater / pHRU->m_areaIrrigated) * MM_PER_M;
            }  // end of: if ( fabs() )
         }  // end of: for each HRU
      }  // end of:    if ( pFlowContext->pEnvContext->yearOfRun > 0 && irrLayerIndex >= 0 && unIrrLayerIndex >= 0 )
   ...
   */
   ...
   //            }
   //      
   //         double targetNonIrrArea = pHRU->m_area - targetIrrigatedArea;
   //         double nonIrrigatedVolume  = pHRU->GetLayer(2)->m_volumeWater;
   //         double irrigatedVolume     = pHRU->GetLayer(3)->m_volumeWater;

   //         double deltaVolume = 0.0;
   //         if (pHRU->m_areaIrrigated > 0.0)
   //            deltaVolume = ABS(irrigatedVolume*(targetIrrigatedArea - pHRU->m_areaIrrigated) / pHRU->m_areaIrrigated);
   //         else if (alglib::fp_eq(targetIrrigatedArea, pHRU->m_area))
   //            deltaVolume = nonIrrigatedVolume;
   ...
   //         {
   //               if (targetIrrigatedArea > pHRU->m_areaIrrigated)
   //               {
   //                  pHRU->GetLayer(3)->m_volumeWater += deltaVolume;
   //                  pHRU->GetLayer(2)->m_volumeWater -= deltaVolume;
   //               }
   //               else if (targetIrrigatedArea < pHRU->m_areaIrrigated)
   //               {
   //                  if (targetIrrigatedArea == 0.0)
   //                  {
   //                     pHRU->GetLayer(2)->m_volumeWater += irrigatedVolume;
   //                     pHRU->GetLayer(3)->m_volumeWater = 0.0;
   //                  }
   //                  else
   //                  {
   //                     pHRU->GetLayer(3)->m_volumeWater -= deltaVolume;
   ...
   //                  }
   //               }
   //               if (targetNonIrrArea > 0.0)
   //                  pHRU->GetLayer(2)->m_wc = (float)(pHRU->GetLayer(2)->m_volumeWater / targetNonIrrArea*1000.0);
   //               if (targetIrrigatedArea > 0.0)
   //                  pHRU->GetLayer(3)->m_wc = (float)(pHRU->GetLayer(3)->m_volumeWater / targetIrrigatedArea*1000.0);
   ...
   if (m_hruLayerNames.GetSize()>3)
      ...
      if (label[0] == name[0])
         ...
         for (int h = 0; h < hruCount; h++)
            ...
            if (targetIrrigatedArea > pHRU->m_area) targetIrrigatedArea = pHRU->m_area;
            float targetNonIrrArea = (float)(pHRU->m_area - targetIrrigatedArea);
            pHRU->GetLayer(3)->m_HRUareaFraction = (float)(targetIrrigatedArea / pHRU->m_area); 
            pHRU->GetLayer(2)->m_HRUareaFraction = 1.f - pHRU->GetLayer(3)->m_HRUareaFraction;
            float nonIrrigatedVolume = (float)pHRU->GetLayer(2)->m_volumeWater;
            float irrigatedVolume = (float)pHRU->GetLayer(3)->m_volumeWater;
            ...
            pHRU->GetLayer(2)->m_volumeWater = pHRU->GetLayer(2)->m_HRUareaFraction * totWater;
            pHRU->GetLayer(3)->m_volumeWater = pHRU->GetLayer(3)->m_HRUareaFraction * totWater;
            pHRU->m_areaIrrigated = (float)targetIrrigatedArea;
      ...
      for ( int h=0; h < hruCount; h++ )
         ...
         for ( int l=0; l < hruLayerCount; l++ )
            ...
            if (gpModel->m_initWaterContent.GetSize()==hruLayerCount)//water content for each layer was specified
               pHRULayer->m_volumeWater = atof(gpModel->m_initWaterContent[l])*pHRULayer->m_depth*0.4f*pHRU->m_area;//assumes porosity is 0.4...we don't know that value at this point in the code, but 0.4 is probably close.
            else if (gpModel->m_initWaterContent.GetSize()==1) // a single value for water content was specified 
               pHRULayer->m_volumeWater = atof(gpModel->m_initWaterContent[0])*pHRULayer->m_depth*0.4f*pHRU->m_area;
            for (int k=0; k < m_hruSvCount-1; k++)   
               {
...
bool FlowModel::WriteDataToMap(EnvContext *pEnvContext )
   ...
   if ( !m_pGrid )   //m_buildCatchmentsMethod != 2 )    // not a grid based model?
      ...
      for ( int h=0; h < hruCount; h++ )
         ...
         for (int k=0; k< pHRU->m_polyIndexArray.GetSize();k++)
            ...
            float snow = (float) pHRU->GetLayer(0)->m_volumeWater/pHRU->m_area*1000.0f;
   ...
   if ( m_pGrid ) // m_buildCatchmentsMethod==2)//a grid based simulation
      ...
      float modelDepth = 0;
      for (int i = 0; i < m_hruLayerDepths.GetSize(); i++)
         modelDepth += (float)atof(m_hruLayerDepths.GetAt(i));
#pragma omp parallel for // firstprivate( pEnvContext )
      for ( int h=0; h < hruCount; h++ )
        {
        HRU *pHRU = m_hruArray[h];
        float waterDepth = 0.0f;
        for (int i = 0; i < pHRU->GetLayerCount(); i++)
           waterDepth += pHRU->GetLayer(i)->m_wDepth;
        float elws = pHRU->m_elevation - modelDepth + (waterDepth/1000.0f);//m
        float depthToWT = pHRU->m_elevation - elws;
         float wc = 0.0f;
         float myElev=50;
         if (m_gridClassification==0)
            wc = pHRU->GetLayer(0)->m_wDepth;
         if (m_gridClassification == 1)
            wc = elws;
         if (m_gridClassification == 2)
            wc = depthToWT;
        if (m_gridClassification==6)//water content in the 1st layer
           wc = pHRU->GetLayer(2)->m_wDepth;
...
bool FlowModel::ResetCumulativeYearlyValues( )
   ...
   for ( int i=0; i < m_hruArray.GetSize(); i++ )
      ...
      for ( int l=0; l < hruLayerCount; l++ )     
         {
         HRULayer *pHRULayer = pHRU->GetLayer( l );
         waterDepth += float(pHRULayer->m_wDepth);//mm of total storage
         }
   ...
   while (reachNdx1 < polyCount - 1)
      ...
      while (reachNdx2 < polyCount)
         ...
         if (comid2 == comid1)
            ...
            float distance2 = pReachPoly2->NearestDistanceToPoly(pReachPoly1);
...
bool FlowModel::GetHRUClimate(CDTYPE type, HRU *pHRU, int dayOfYear, float &value)
   ...
   if (!m_useStationData)
      ...
      if (!m_estimateParameters)
         ...
         if (pInfo == NULL || pInfo->m_pDataObj == NULL) OKflag = false;  // no data object
         else
            {
            if (pHRU->m_climateIndex < 0)
               ...
               value = pInfo->m_pDataObj->Get(x, y, pHRU->m_climateIndex, dayOfYear, m_projectionWKT, false, &OKflag);   // units?????!!!!
               }
            else
               {
         ...
         } // end of if (!m_estimateParameters)
         ...
         if (pInfo == NULL || pObj == NULL) OKflag = false;  // no data object
         else
            {
            if (pHRU->m_climateIndex < 0)
               {
               double x = pHRU->m_centroid.x;
               double y = pHRU->m_centroid.y;
               value = pObj->Get(x, y, pHRU->m_climateIndex, dayOfYear, m_projectionWKT, false, &OKflag);   // units?????!!!!
               }
            else
               {
               value = pObj->Get(pHRU->m_climateIndex, dayOfYear);
               }
            } // end of if (pInfo == NULL || pObj == NULL) ... else ...
         } // end of if (!m_estimateParameters) ... else ...
      } // end of if (!m_useStationData)
   else if (m_pClimateStationData == NULL) OKflag = false;
   else
      {
      switch (type)
         {
      ...
      } // end of if (!m_useStationData) ... else if (m_pClimateStationData == NULL) ... else ...

   if (!OKflag)
      {
      CString msg;
      msg.Format("*** GetHRUClimate() failed. type = %d, m_id = %d, dayOfYear = %d", type, pHRU->m_id, dayOfYear);
      Report::LogMsg(msg);
      }
...
bool FlowModel::GetReachClimate(CDTYPE type, Reach *pReach, int dayOfYear, float &value)
   ...
   if (pInfo != NULL && pInfo->m_pDataObj != NULL)   // find a data object?
      ...
      if (pReach->m_climateIndex < 0)
         ...
         if (pPoly->GetVertexCount() > 0)
            {
            x = pPoly->GetVertex(0).x;  //pPoly->GetCentroid().x;
            y = pPoly->GetVertex(0).y;   //pPoly->GetCentroid().y;
            value = pInfo->m_pDataObj->Get(x, y, pReach->m_climateIndex, dayOfYear, m_projectionWKT, false, &OKflag);   // units?????!!!!
            }
         else
            {
            CString msg;
   ...
   if (!OKflag)
      {
      CString msg;
      msg.Format("*** GetReachClimate() failed. type = %d, m_reachID = %d, dayOfYear = %d, x = %f, y = %f, pReach->m_climateIndex = %d",
         type, pReach->m_reachID, dayOfYear, x, y, pReach->m_climateIndex);
      Report::LogMsg(msg);
      }

   return(OKflag);
   } // end of GetReachClimate()
...
bool FlowModel::CollectModelOutput(void )
   ...
   if ( moCountHru > 0 )
      ...
      for ( int h=0; h < (int) this->m_hruArray.GetSize(); h++ )
         ...
         HRU::m_mvTopSoilH2O_mm = ((pHRU->GetLayer(2)->m_volumeWater + pHRU->GetLayer(3)->m_volumeWater) / pHRU->m_area)*1000.f;
         HRU::m_mvShallowGW_mm = (pHRU->GetLayer(4)->m_volumeWater / pHRU->m_area)*1000.f;
         HRU::m_mvDeepGW_mm = (pHRU->GetLayer(5)->m_volumeWater / pHRU->m_area)*1000.f;
            ...
            for ( int j = 0; j < (int)m_modelOutputGroupArray.GetSize(); j++ )
            {
               ...
               for ( int i = 0; i < (int)pGroup->GetSize(); i++ )
               {
                  ...
                  if ( pOutput->m_inUse == true && pOutput->m_modelDomain == MOD_HRULAYER )
                  {
                     ...
                     HRULayer::m_mvWDepth = pHRULayer->m_wDepth;
   ...
   if ( moCountReach > 0 )
      ...
      for ( int h=0; h < (int) this->m_reachArray.GetSize(); h++ )
         ...
         int streamNdx = pStreamLayer->FindIndex(m_colStreamCOMID, pReach->m_reachID);
         ...
         Reach::m_mvREACH_H2O = pReach->GetDepthFromQ(pReach->m_availableDischarge, pReach->m_wdRatio)*pReach->m_length*pReach->m_width;
