...
runfile('C:\Users\Joy Hill\Desktop\BPA_stuff\Northwest Region\Williamette\ResSim Reservoirs\Willamette\Williamette_model.py', wdir='C:\Users\Joy Hill\Desktop\BPA_stuff\Northwest Region\Williamette\ResSim Reservoirs\Willamette')
runfile('C:\Users\Joy Hill\Desktop\BPA_stuff\Northwest Region\Williamette\ResSim Reservoirs\Willamette\Willamette_outer.py', wdir='C:\Users\Joy Hill\Desktop\BPA_stuff\Northwest Region\Williamette\ResSim Reservoirs\Willamette')
                      ...
                      if str(name.ID) in cp_name.influencedReservoirs:  #Make sure that the reservoir is on the influenced reservoir list
                          ...
                          if constraint_array[i].startswith('cp_Max'):  #maximum    
                              if cp_discharge[t-1,cp_id-1] > constraintValue:   #Are we above the maximum flow?   
                                  resallocation = (constraintValue - cp_discharge[t-1,cp_id-1])/len(cp_name.influencedReservoirs) #Allocate decrease in releases (should be negative) over "controlled" reservoirs if maximum, evenly for now
                              else:  
                                  resallocation = 0
                              ...
                              if cp_discharge[t-1,cp_id-1] < constraintValue:   #Are we below the minimum flow?  
                                 resallocation = (constraintValue - cp_discharge[t-1,cp_id-1])/len(cp_name.influencedReservoirs) 
                              else:  
                                  resallocation = 0
